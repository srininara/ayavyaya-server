...
def get():
  frequency_datewise_expenses_tuple_list = db.session.query(
    Expense_Frequency.name.label("frequency"),Expense.expense_date,db.func.sum(Expense.amount).label("frequency_expenses")).join(Expense).group_by(
        Expense_Frequency.id).group_by(Expense.expense_date).order_by(Expense.expense_date).order_by(db.asc("frequency")).all()
