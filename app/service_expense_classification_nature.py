...
def get():
  nature_datewise_expenses_tuple_list = db.session.query(
    Expense_Nature.name.label("nature"),Expense.expense_date,db.func.sum(Expense.amount).label("nature_expenses")).join(Expense).group_by(
        Expense_Nature.id).group_by(Expense.expense_date).order_by(Expense.expense_date).order_by(db.asc("nature")).all()
